...
import com.github.aakumykov.kotlin_playground.extensions.errorMsg
...
import com.github.aakumykov.kotlin_playground.utils.humanReadableByteCount
import com.github.aakumykov.kotlin_playground.utils.random
...
//                    Log.d(TAG, "speed: $progress")
            ...
            setProgressLabelProvider { progress ->
                getString(R.string.speed, progress, humanReadableByteCount(progress.toLong(), decimalNotation=true))
            }
        }
    }
    ...
    private fun onStartClicked() {
        ...
        Log.d(TAG, "@@ размер данных: ${humanReadableByteCount(dataSizeBytes.toLong())}, " +
                "скорость: ${humanReadableByteCount(speedBytesPerSec.toLong(), decimalNotation = true)}/с, " +
    ...
    fun doCopy(iStream: InputStream, oStream: OutputStream, estimatedTimeMs: Int) {
        copyBetweenStreamsWithSpeed(
            ...
            progressCallback = { totalBytesTransferred, speed ->
                if (isDebugEnabled)
                    Log.d(TAG, "totalBytesTransferred: $totalBytesTransferred, speed: ${humanReadableByteCount(speed.toLong())}/s")
